...
(function ($) {
	var lc_NewsBox = function(element, lcnb_settings) {
		...
		var fetch_news = function(index, type, src, params, src_opts) {
			...
			// fetch
			var dataType = (type == "facebook" || type == "twitter" || type == "soundcloud") ? "json" : "jsonp";
			$.ajax({
				url: src,
				data: params,
				dataType: dataType,
				...
				success: function(resp) {
					...
					if(!error && typeof(resp) != 'undefined') {
						$.each(resp, function(i, news) {
							switch(type) {
								case 'facebook' : 
									var d = new Date(news.created_time);
								...
								case 'youtube' : 
									if(news.snippet.type == 'upload') {
										...
										var thumb_url = video_thumb.replace('/maxresdefault.jpg', '');
	...
	$.fn.lc_news_box = function(lcnb_settings) {
		...
		$.fn.lcnb_paginate = function(direction) {
			var $elem = $(this);
